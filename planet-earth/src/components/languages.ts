// src/data/languages.ts
export const content = {
  EN: {
    heroTitle: "Paloma Guth Kronbauer",
    heroSub: "Computer Science & Mathematics @ St. Olaf College",
    about: "The Archive",
    bio: "Data Engineer and Backend Developer specialized in Linux environments, Python automation, and optimizing systems for scalability and security.",
    education: "The Academy",
    edu1Title: "B.A. in Computer Science and Mathematics",
    edu1Place: "St. Olaf College | 2022 - 2026",
    edu2Title: "Neural Networks & Deep Learning",
    edu2Place: "DIS Denmark | 2025",
    eduIB: "International Baccalaureate (IB) Diploma",
    projects: "Active Quests",
    activeQuests: "Active Quests",
    experience: "Adventure History",
    job1Title: "Technical Lead & Mentor",
    job1Place: "St. Olaf Tech Hub | 2022 - Present",
    job2Title: "IT Support Analyst",
    job2Place: "St. Olaf IT | 2023 - 2024",
    skills: "Prapared Spells",
    inventory: "Inventory & Achievements",
    lvl: "Lvl.",
    spellbookTitle: "The Path of Mastery",
    levelDisclaimer: "* Expertise measured on a scale of Level 1 to Level 10",
    flavorQuote: "“Information is the mana of the modern age; logic is the circle through which we channel it.”",
    flavorAuthor: "— The Data Paladin's Creed",
    cantripTitle: "Base Cantrips (Soft Skills)",
    lvlGroupTitle: "Level {n} Spells",
    commName: "Technical Communication",
    pythonName: "Python & Automation",
    sqlName: "SQL & Data Modeling",
  },
  PT: {
    heroTitle: "Paloma Guth Kronbauer",
    heroSub: "Ciência da Computação e Matemática @ St. Olaf College",
    about: "Arquivo",
    bio: "Engenheira de Dados e Desenvolvedora Backend especializada em ambientes Linux, automação em Python e otimização de sistemas para escalabilidade e segurança.",
    education: "Academia",
    edu1Title: "Bacharelado em Ciência da Computação e Matemática",
    edu1Place: "St. Olaf College | 2022 - 2026",
    edu2Title: "Redes Neurais e Deep Learning",
    edu2Place: "DIS Denmark | 2025",
    eduIB: "Diploma de Bacharelado Internacional (IB)",
    projects: "Missões Ativas",
    activeQuests: "Missões Ativas",
    experience: "Histórico de Aventuras",
    job1Title: "Gestora e Mentora Técnica",
    job1Place: "Hub de Tecnologia St. Olaf | 2022 - Atual",
    job2Title: "Analista de Suporte de TI",
    job2Place: "Suporte de TI St. Olaf | 2023 - 2024",
    skills: "Atributos",
    inventory: "Inventário e Conquistas",
    lvl: "Nível",
    spellbookTitle: "A Trilha da Maestria",
    levelDisclaimer: "* Experiência medida em uma escala de Nível 1 a Nível 10",
    flavorQuote: "“A informação é o mana da era moderna; a lógica é o círculo pelo qual o canalizamos.”",
    flavorAuthor: "— O Credo do Paladino de Dados",
    cantripTitle: "Cantos Iniciais (Soft Skills)",
    lvlGroupTitle: "Magias de Nível {n}",
    commName: "Comunicação Técnica",
    commDesc: "Articulando fluxos de dados complexos para stakeholders.",
    pythonName: "Python e Automação",
    sqlName: "SQL e Modelagem de Dados",
  }
};